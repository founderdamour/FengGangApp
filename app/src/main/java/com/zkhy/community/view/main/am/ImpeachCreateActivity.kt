 ...
 * <pre>
 */
class ImpeachCreateActivity : TitleBarBaseActivity(), View.OnClickListener, AndroidExt2View {

    var presenter: AMPresenter? = null

    override fun getLayout(): Int = R.layout.activity_am_impeach_submit

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setTitleBar("网上信访", "我的信访") {
            IntentUtil.openActivity(this, ImpeachHistoryListActivity::class.java)
                .start()
        }
    ...
    override fun onClick(v: View?) {
        when (v) {
            userAddressItem -> {
                ...
                InfoEditActivity.start(this@ImpeachCreateActivity, keyValue)
            }
            titleItem -> {
                val keyValue = KeyValueEntity()
                keyValue.title = "信访标题"
                keyValue.content = titleValueTv.text.toString()
                keyValue.inputHint = "请填写信访标题"
    ...
    private fun submitImpeach() {
        ...
        if (StringUtil.isEmpty(titleValue)) {
            ToastUtil.show("请正确输入信访标题")
            return
        }

        if (StringUtil.isEmpty(contentValue)) {
            ToastUtil.show("请正确输入信访内容")
            return
        }

        val entity = ImpeachEntity()
        ...
        presenter?.createImpeach(entity)
    }
