package com.zkhy.fenggang.community.view.photos.fragment;
...
import com.zkhy.fenggang.community.presenter.PhotosCreatorPresenter
import com.zkhy.fenggang.community.view.photos.ShotPhotoDetailActivity
import com.zkhy.fenggang.community.view.photos.adapter.MyShotPhotoListAdapter
 ...
 */
class MyShotPhoto4DoingListFragment : RecycleViewBaseFragment(), AndroidExt2View {

    private var pageNo: Int = 1
    private var adapter: MyShotPhotoListAdapter? = null
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        adapter = MyShotPhotoListAdapter(this.context!!, ArrayList())
        loadingRecyclerView.adapter = adapter
        adapter!!.setClickCallBack(ItemClickCallBack { _, entity ->
            IntentUtil.openActivity(activity, ShotPhotoDetailActivity::class.java)
    ...
    private fun loadData(pageNo: Int) {
        ...
        pCreatorVo.userId = userId

        val stateList = ArrayList<Int>()
        stateList.add(0)
        stateList.add(1)
        pCreatorVo.stateList = stateList
